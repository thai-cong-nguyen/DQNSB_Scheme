...
def MulticastCommittee(commList, identity, txns):
    """
    ...
    for committee_id in commList:
        ...
        for memberId in commMembers:
            # union of committee members views
            data = {"committee members" : commMembers , "final Committee members"  : finalCommitteeMembers , "txns" : txns[committee_id] ,"identity" : identity}
...
class Identity:
    ...
    def __str__(self) -> str:
        return "\nIdentity - IP: {} - PK: {} - committee_id: {} - PoW: {} - epoch_randomness: {} - part_of_network: {} - port: {}".format(self.IP, self.PK, self.committee_id, self.PoW, self.epoch_randomness, self.partOfNtw, self.port)

    def isEqual(self, identity):
        """
            checking two objects of Identity class are equal or not
        """
        return self.IP == identity.IP and self.PK == identity.PK and self.committee_id == identity.committee_id \
        and self.PoW == identity.PoW and self.epoch_randomness == identity.epoch_randomness and self.partOfNtw == identity.partOfNtw and self.port == identity.port
...
class Elastico:
    ...
    def get_key(self):
        ...
        key_pem = key.export_key().decode('utf-8')  # Export to PEM format
        return key_pem
    ...
    def compute_PoW(self):
        ...
        if self.state == ELASTICO_STATES["NONE"]:
            ...
            PK = RSA.import_key(self.key).public_key().export_key().decode()
            IP = self.IP
    ...
    def is_OwnIdentity(self, identity):
        """
            Checking whether the identity is the Elastico node's identity or not
        """
        if self.identity == "":
            self.form_identity()
        return self.identity.isEqual(identity)
            ...
            if msg["type"] == "directoryMember":
                ...
                if self.verify_PoW(identity):
                    if len(self.cur_directory) < c:
                     ...
                     logging.info("%s  PoW not valid of an incoming directory member " , str(identity))
            elif msg["type"] == "newNode" and self.is_directory:
                identity = msg["data"]
                if self.verify_PoW(identity):
                    if identity.committee_id not in self.committee_list:
                        # Add the identity in committee
                        self.committee_list[identity.committee_id] = [identity]

                    elif len(self.committee_list[identity.committee_id]) < c:
                        # Add the identity in committee
                        self.committee_list[identity.committee_id].append(identity)

                        if len(self.committee_list[identity.committee_id]) == c:
    ...
    def executeServer(self, nodeIndex):
        ...
        """
        try:
            logging.info("Socket connection - %s", self.socketConn)
            print("Socket connection:", self.socketConn)
            s = self.socketConn
            port = self.port
            ...
            logging.info("Node is serving - %s", self.serve)
            while self.serve:
...
def executeSteps(network_nodes, nodeIndex, epochTxn):
    """
        A process will execute the elastico node
    """
    # global network_nodes
    logging.info("Running node :- %s", nodeIndex)
    logging.info("Network nodes :- %s", network_nodes)
    try:
        if nodeIndex >= len(network_nodes):
            logging.error(f"Node index {nodeIndex} out of range. Total nodes: {len(network_nodes)}")
            return
        node = network_nodes[nodeIndex]
        logging.info("Node details: %s", node)
        ...
        serverThread.start()
        logging.info("Executing step for node: %s", nodeIndex)
        while True:
            # execute one step of elastico node
            response = node.execute(epochTxn)
            # logging.info("Response from node: %s", response)
            if response == "reset":
                # now reset the node
                if isinstance(node.identity, Identity):
                    ...
                    print("illegal call")
                    logging.warning("illegal call - reset node...")
...
def Run(epochTxn):
    ...
    if len(network_nodes) == 0:
        ...
        for i in range(n):
            print( "\n---Running for processor number---" , i + 1)
            elastico = Elastico()
    ...
    # executeSteps(network_nodes, 0, epochTxn)
    # return
    ...
    for nodeIndex in range(n):
        # create a process
        # temp_network_nodes = [node for node in network_nodes]
        # network_nodes = temp_network_nodes
        process = Process(target=executeSteps, args=(network_nodes, nodeIndex, epochTxn))
        # add to the list of processes
        processes.append(process)

    for nodeIndex in range(n):
        print("process number " , nodeIndex , "started")
        logging.info("process number :- %s started" , nodeIndex)
        # start the process
        processes[nodeIndex].start()

    for nodeIndex in range(n):
        logging.info("process number :- %s waited to finish" , nodeIndex)
        # waits for the process to finish
        processes[nodeIndex].join()

    # All processes are over. Computing response in each node to update ledger
    for nodeIndex in range(n):
        logging.info("response number :- %s", nodeIndex)
...
if __name__ == "__main__":
    try:
        ...
        logging.basicConfig(filename='./src/server/data/elastico.log',filemode='w',level=logging.DEBUG)
        ...
        for i in range(numOfEpochs):
            ...
            epochTxns[i] = txns
        # run all the epochs 
        logging.info("len epochTxns := %s", len(epochTxns))
